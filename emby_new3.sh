...
if [ -s "$config_dir/emby_config.txt" ];then
	...
	echo 'version=4.9.0.20' >> "$config_dir/emby_config.txt"
...
if [ "$action" == "generate_config" ]; then
	...
	echo 'bash -c "$(curl https://docker.xiaoya.pro/emby_new.sh)" -s' "--config_dir=$config_dir"
	exit
fi
...
if [ -z "$docker_exist" ]; then
	echo "$emby_image 拉取镜像失败，请检查网络，或者翻墙后再试11"
	#exit 1
fi
...
if [ ! "$local_sha" == "$remote_sha" ]; then
	...
	if [ -z "$glue_exist" ]; then
		echo "xiaoyaliu/glue:latest 拉取镜像失败，请检查网络，或者翻墙后再试22"
		exit 1
	fi	
fi
...
case $cpu_arch in
	"x86_64" | *"amd64"*)
		if [ $mode == "host" ]; then
			docker run -d --name emby -v $media_dir/config:/config -v $media_dir/xiaoya:/media -v $config_dir/nsswitch.conf:/etc/nsswitch.conf --network=host --add-host="xiaoya.host:$xiaoya_host" $gpu --user 0:0 -v UID=0 -v GID=0 -v GLIST=0 --restart always xinjiawei1/emby_unlockd:latest
		fi
		if [ $mode == "bridge" ]; then
			docker run -d --name emby -v $media_dir/config:/config -v $media_dir/xiaoya:/media -v $config_dir/nsswitch.conf:/etc/nsswitch.conf -p 6908:6908 --add-host="xiaoya.host:$xiaoya_host" $gpu --user 0:0 -v UID=0 -v GID=0 -v GLIST=0 --restart always xinjiawei1/emby_unlockd:latest
		fi
		echo "一键全家桶全部安装完成"
		;;
	"aarch64" | *"arm64"* | *"armv8"* | *"arm/v8"*)
...
if [ "$resilio" == "yes" ];then	
	...
	bash -c "$(curl https://docker.xiaoya.pro/resilio.sh)" -s $media_dir $config_dir
fi
